...
export default function App() {
  ...
  const LL = createMemo<TranslationFunctions>(() => i18n(locale()));

  const telemetry = createMemo(() => [
    {
      label: LL().telemetry.temperature(),
      value: temperature,
      unit: LL().units.celsius(),
      format: (n: number) => n.toFixed(1),
    },
    {
      label: LL().telemetry.rpm(),
      value: rpm,
      unit: '',
      format: (n: number) => n.toFixed(0),
    },
    {
      label: LL().telemetry.pressure(),
      value: pressure,
      unit: LL().units.kpa(),
      format: (n: number) => n.toFixed(1),
    },
    {
      label: LL().telemetry.flowRate(),
      value: flowRate,
      unit: LL().units.litersPerMin(),
      format: (n: number) => n.toFixed(2),
    },
    {
      label: LL().telemetry.voltage(),
      value: voltage,
      unit: LL().units.volts(),
      format: (n: number) => n.toFixed(2),
    },
  ...
  return (
    <main class="max-w-120 mx-auto p-4 flex flex-col gap-4 font-sans bg-gray-100 min-h-screen text-gray-900">
      <header class="flex justify-between items-center py-2">
        <h1 class="text-xl font-semibold">{LL().dashboard.title()}</h1>
        <div class="flex items-center gap-4">
          ...
          <div class="flex items-center gap-2">
            ...
            <span class="text-sm text-gray-500">
              {connected() ? LL().dashboard.connected() : LL().dashboard.disconnected()}
            </span>
          </div>
        </div>
      </header>
      ...
      <section class="bg-white rounded-lg p-4 shadow-sm">
        ...
        <div class="grid grid-cols-3 gap-2">
          ...
          >
            ...
            <span class="text-xs font-medium">{LL().controls.heater()}</span>
            <span class="text-[10px] uppercase tracking-wide opacity-80">
              {heaterOn() ? LL().controls.on() : LL().controls.off()}
            </span>
          </button>
          <button
            class={`flex flex-col items-center gap-1 py-3 px-2 border rounded-lg cursor-pointer transition-all
          ...
          >
            <span class="text-xl">💧</span>
            <span class="text-xs font-medium">{LL().controls.pump()}</span>
            <span class="text-[10px] uppercase tracking-wide opacity-80">
              {pumpOn() ? LL().controls.on() : LL().controls.off()}
            </span>
          </button>
          <button
            class={`flex flex-col items-center gap-1 py-3 px-2 border rounded-lg cursor-pointer transition-all
          ...
          >
            <span class="text-xl">🔌</span>
            <span class="text-xs font-medium">{LL().controls.connection()}</span>
            <span class="text-[10px] uppercase tracking-wide opacity-80">
              {connected() ? LL().controls.disconnect() : LL().controls.connect()}
            </span>
          </button>
        </div>
      </section>
